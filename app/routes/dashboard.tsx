...
export default function App() {
  ...
  const { data, supabaseUrl, supabaseKey } = useLoaderData<{
    data: Title[];
    supabaseUrl: string;
    supabaseKey: string;
  }>();
      ...
      <div className="font-sans w-3/4 mt-16">
        ...
        <div className="flex flex-row mt-2 gap-[10px] w-full">
          <button className="flex items-center bg-emerald-500 hover:bg-emerald-600 hover:text-slate-200 active:bg-emerald-700 active:text-slate-300 px-3 py-2 rounded-[3px]">
            <span className="mr-2 font-sans text-sm font-bold w-16">
              Add File
            </span>
            <FaPlus />
          </button>
        ...
        <div className="flex flex-row mt-5 gap-[10px] w-full">
          <div className="flex flex-col gap-4 dashboard-opacity w-full p-4 rounded-[3px]">
            <div className="flex flex-row items-center gap-4">
              <span className="w-1/3 text-right font-semibold text-base">
                Title
              </span>
              <div className="relative w-full">
                <input
                  ...
                  className={`bg-white bg-opacity-20 px-3 py-2 w-full text-sm font-bold rounded-[3px] font-sans ${
                    titleError ? "border-rose-500 border" : ""
                  }`}
                  value={title}
                  onChange={(e) => setTitle(e.target.value)}
                />
                {titleError && (
                  <FaCircleExclamation className="absolute right-2 top-1/2 transform -translate-y-1/2 text-rose-500" />
                )}
              </div>
            </div>
            <div className="flex flex-row items-center gap-4">
              ...
              <div className="relative w-full">
                ...
                >
                  <span className="mr-2 font-sans text-sm font-bold">
                    Attach
                  </span>
                  <FaLink />
                </button>
                ...
                />
                {fileError && (
                  <FaCircleExclamation className="absolute right-2 top-1/2 transform -translate-y-1/2 text-white" />
                )}
              </div>
            </div>
            {fileName && (
              <div className="flex justify-end">
                <div
                  className={`w-32 h-40 rounded-[5px] flex flex-col justify-between items-center ${
                    isHovered
                ...
                >
                  <div className="flex-grow flex items-center justify-center">
                    {isHovered ? (
                      <IoMdRemoveCircle size={60} color="rgb(244, 63, 94)" />
                    ) : (
                      <RiFile2Fill size={60} />
                    )}
                  </div>
                  <div
                    className={`flex justify-center items-end w-full rounded-b-[5px] ${
                      isHovered
                        ? "bg-rose-500 bg-opacity-40"
                        : "bg-white bg-opacity-20"
                    }`}
            ...
            <button
              ...
              className="flex items-center justify-center bg-emerald-500 hover:bg-emerald-600 hover:text-slate-200 active:bg-emerald-700 active:text-slate-300 px-3 py-2 rounded-[3px]"
            >
              <span className="mr-2 font-sans text-sm font-bold">
                Upload File
              </span>
